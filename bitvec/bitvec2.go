...
func (this *BasicBitVector) Rank1(index int) int {
	...
	var rank3 uint = 0
	for i := 0; i < by; i++ {
		rank3 += uint(this.popc[this.bitArr.arr[i+c*4+b*this.subBlockNum*4]])
	}
	rank3 += uint(this.bitArr.RangeRank1(8*(by+c*4+b*this.subBlockNum*4), r-1+8*(by+c*4+b*this.subBlockNum*4)))
